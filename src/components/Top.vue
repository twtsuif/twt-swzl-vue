<template>
  ...
  <div id="total">
    ...
    <div class="menu1" v-show="isShowCampus">
      <div
          :class="checkedBei ? 'background-white' : 'background-green'"
          @click="checkBei"
      >
        <div :class="checkedBei ? 'word-green' : 'word-white'">北洋园</div>
      </div>
      <div
          :class="checkedWei ? 'background-white' : 'background-green'"
          @click="checkWei"
      >
...
export default {
  ...
  methods: {
    ...
    timeClick(date, which) {
      ...
      if (date === 7) {
        this.$store.commit("SET_DATE", "earlier");
      } else {
    ...
    //更换种类id
    checkCategory() {
      this.$store.commit("SET_CATEGORY_ID", this.categoryId);
    },
    // 更换校区
    checkCampus(campus) {
